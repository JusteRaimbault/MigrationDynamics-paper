...
Intra-urban residential dynamics : empirical study in \cite{wu2006migrant} \comment{Why did you add this sentence here?}[unfinished biblio listing\ldots]
...
\item Cumulated utility difference in migrations \comment{(CL) What does that mean ?}[when someone migrates, it corresponds to a given utility gain $\Delta U$ : this indicator is $\sum \Delta U$ on all realized migrations.]
...
We summarise main parameters in table \comment{which table ?}[not there yet...].
...
\item Adding categorisation does not change the qualitative behavior of the model. \comment{Do you mean that categorization (based on professional qualifications, education, and economic situation) does not influence the migration behavior? I think that for example the points-based hukou system favoring “talents” – people with more qualifications, with stable job and economic situation etc. – can have a relevant influence on migration behavior.}[It changes the migration behavior for individuals, but not the global qualitative picture of the previous stylized facts. I have to think more on this and I totally agree on your second point : an interesting thing to try when testing the hukou policies in the model is to check if we have the same phenomenon, and then to compare the effectiveness of policies at the individual level (impact on migrants trajectories) to the global level (behavior of the model).] The lower category seems more vulnerable to spatial inequalities created by social determinism.
\end{itemize}
...
\item what is the impact of various spatial distribution of $h_j^{(c)}$, i.e. different government policies ? \comment{(Juste) Sur ce point le mieux serait de tester les politiques que tu as pu mettre en évidence, de manière scénarisée ?}
\item Comparison with and without Hong-Kong and Macao \comment{(Juste) I'm not sure this point is a good idea, as interactions processes are quite different (e.g. daily commuting, different migration rules, etc.) and it would become a gaz factory to include them in the model.}
\end{itemize}
\comment{(CL) qui a fait ces commentaires ?}[(JR) Moi]
